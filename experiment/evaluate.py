...
def get_model(model_type, **kwargs):
    if model_type == 0:
        ...
        model = Model(
            input_dim=kwargs['esm_dim'], hidden_dim=kwargs['hidden_dim'], num_classes=kwargs['num_classes'], 
            prototype_index=kwargs['prototype_index'], proto_idx_mask=kwargs['proto_idx_mask'], go_freq=kwargs['go_freq']
        ).to(kwargs['device'])
        model.init_prototype_feats(kwargs['num_classes'], kwargs['hidden_dim'], kwargs['stacked_feats'])
...
def main(args, config):
    ...
    model = get_model(model_type=model_type, esm_dim=esm_dim, hidden_dim=hidden_dim, num_classes=num_classes, prototype_index=prototype_index, proto_idx_mask=proto_idx_mask, go_freq=go_freq, stacked_feats=stacked_feats, device=device)
