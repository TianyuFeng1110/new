...
def valid(model, loader, epoch, device, scale, hier_reg_lambda, parent_indices, child_indices):
    ...
    for batch_idx, (batch_residue_feats, mask, labels, indices) in enumerate(metric_logger.log_every(loader, print_freq=1, header=header)):
...
def train(model, optimizer, loader, epoch, device, scale, hier_reg_lambda, parent_indices, child_indices):
    ...
    for batch_idx, (batch_residue_feats, mask, labels, indices) in enumerate(metric_logger.log_every(loader, print_freq=1, header=header)):
        ...
        logits, custom_logits, custom_loss, proto_loss, gate_loss, sigma = model(batch_residue_feats.to(device), indices.to(device), mask.to(device), labels.to(device))
        hier_loss = utils.compute_hier_loss(custom_logits, parent_indices, child_indices)
        loss = (1 - hier_reg_lambda) * (scale['custom_scale'] * custom_loss + scale['proto_scale'] * proto_loss + scale['gate_scale'] * gate_loss) + hier_reg_lambda * (scale['hier_scale'] * hier_loss)
...
def main(args, config):
    ...
    # protein_feats = torch.stack([protein_feats[k] for k in sorted(protein_feats.keys(), key=int)])
    ...
    residue_feats_list = [residue_feats[k] for k in sorted(residue_feats.keys(), key=int)]
    model = Model(
    ...
    model.init_protein_feats(residue_feats_list, device, batch_size=batch_size)
    ...
    optimizer = optim.AdamW([
        {'params': prototype_params, 'lr': base_lr * 6.0}, # 原型模块
