...
def valid(model, loader, epoch, device, batch_size):
    ...
    metric_logger.add_meter('sim_loss', utils.SmoothedValue(window_size=50, fmt='{value:.4f}'))
    ...
    alpha_sim, alpha_proto, alpha_gate, scale = 0.4, 1, 6, (1 / batch_size) # 比例系数
    for batch_idx, (data, labels) in enumerate(metric_logger.log_every(loader, print_freq=1, header=header)):

        probs_mlp, _, probs, sim_loss, proto_loss, gate_loss, tau, bias, k, c = model(data.to(device), labels.to(device))
        
        loss = (alpha_sim * sim_loss + alpha_proto * proto_loss + alpha_gate * gate_loss) * scale
        all_probs.append(probs.detach().cpu())
        all_probs_mlp.append(probs_mlp.detach().cpu())
        all_labels.append(labels.detach().cpu())
        metric_logger.update(total_loss=loss.item())
        metric_logger.update(sim_loss=sim_loss.item() * alpha_sim * scale)
...
def train(model, optimizer, loader, epoch, device, batch_size):
    ...
    for batch_idx, (data, labels) in enumerate(metric_logger.log_every(loader, print_freq=1, header=header)):
        ...
        _, _, _, sim_loss, proto_loss, gate_loss, tau, bias, k, c  = model(data.to(device), labels.to(device))
