...
from models.cc_model import CustomModel
...
def valid(model, loader, epoch, device, scale, hier_reg_lambda, parent_indices, child_indices):
    ...
    for batch_idx, (protein_feats, labels, indices) in enumerate(metric_logger.log_every(loader, print_freq=1, header=header)):
        ...
        custom_probs, custom_logits, custom_loss = model(protein_feats.to(device), labels.to(device))
        hier_loss = utils.compute_hier_loss(custom_logits, parent_indices, child_indices)
        loss = (1 - hier_reg_lambda) * custom_loss + hier_reg_lambda * (scale['hier_scale'] * hier_loss)

        all_probs.append(custom_probs.detach().cpu())  # final_probs 已经是概率值，不需要再 sigmoid
        all_labels.append(labels.detach().cpu())
        metric_logger.update(total_loss=loss.item())
        metric_logger.update(custom_loss=custom_loss.item())
        metric_logger.update(hier_loss=scale['hier_scale'] * hier_loss.item())
...
def main(args, config):
    ...
    hier_lambda_warmup = config.get('hier_lambda_warmup', 10)   # 多少个 epoch 内线性增长
    hier_lambda_max = config.get('hier_lambda_max', 0.5)        # λ 增长的上限
    ...
    scale = {'hier_scale': hier_scale}
    
    print('start traing......')
    for epoch in range(epochs):

        # 动态调整 hier_reg_lambda：线性增长到 hier_lambda_max
        effective_lambda = min(hier_reg_lambda * (epoch + 1) / hier_lambda_warmup, hier_lambda_max)

        # 训练
        train(model, optimizer, train_loader, epoch, device, scale, effective_lambda, parent_indices, child_indices)
        scheduler.step()

        # 验证
        all_probs, all_labels = valid(model, valid_loader, epoch, device, scale, effective_lambda, parent_indices, child_indices)
    ...
    # utils.draw_frequencies_AUPRC(torch.cat(all_probs, dim=0).numpy(), torch.cat(all_probs_averge, dim=0).numpy(), torch.cat(all_labels, dim=0).numpy(), valid_freq, save_path=result_path, namespace=namespace) # 训练结束绘制结果曲线图
        ...
        torch.save(save_obj, os.path.join("/archive/hot5/fty/checkpoints/step1/", 'checkpoint_%02d.pth'%epoch))  
