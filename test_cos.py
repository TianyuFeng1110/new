...
def valid(model, prototypes, valid_loader, epoch, device):
    ...
    # 打印可学习参数 lambda/beta/smooth_tau 的当前实际值，观察学习进度
    lam, beta, tau = model.get_smooth_params()
    print("Averaged stats(Valid): {}, Fmax: {:.4f}, micro AUPRC: {:.4f}, lambda: {:.4f}, beta: {:.4f}, tau: {:.4f}".format(
        metric_logger.global_avg(), metric['Fmax'], metric['micro_AUPRC'], lam, beta, tau))
...
def main(args, config):
    ...
    tau_min = float(config.get('tau_min', 0.0))  # 可学习 tau 的下界，防止罕见类原型坍缩到单样本；0 表示不限制
    ...
    if dataset_name == 'TALE':
        ...
        if not is_zero_shot: ic = ic[valid_mask]
    # lambda/beta/smooth_tau 为模型内可学习参数，初始值由数据分布自动估计（无需手工设置）
    lambda_init, beta_init, tau_init = utils.compute_smooth_param_inits(hop_counts, ic, class_counts)
    print(f'可学习参数初始值(数据驱动): lambda={lambda_init:.4f}, beta={beta_init:.4f}, tau={tau_init:.4f}')
    ...
    # ---- 模型 ----
    # lambda/beta/smooth_tau 为可学习参数，初始值由数据分布自动估计
    model = PrototypeNet(
        ...
        smooth_tau=tau_init, lambda_init=lambda_init, beta_init=beta_init, tau_min=tau_min).to(device)
